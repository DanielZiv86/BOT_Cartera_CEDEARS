...
def test_visa_routes_to_corporate_not_bank_balance_sheet_model():
 ...
 assert r['scenario_sector_model']=='CORPORATE'; assert r['scenario_sector_source']=='POLICY_OVERRIDE'; assert r['bear_target_price']<r['base_target_price']<r['bull_target_price']; assert r['scenario_validated']
...
def test_verified_adr_ratio_normalization_can_reconcile_identity_and_financial_scenario_units():
 ...
 assert r['economic_identity_status']=='VERIFIED_NORMALIZED'; assert r['identity_adr_ratio_applied']==5; assert r['identity_adr_ratio_verified']; assert r['scenario_validated']; assert r['base_target_price']>70; assert 'SCENARIO_FUNDAMENTALS_NORMALIZED_TO_TRADED_SECURITY' in r['scenario_review_flags']
...
def test_vod_negative_eps_uses_pb_identity_fallback():
 ...
 assert r['economic_identity_status']=='VERIFIED_NORMALIZED'; assert r['economic_identity_method']=='PB'; assert r['identity_adr_ratio_applied']==10; assert 'ECONOMIC_IDENTITY_PB_FALLBACK_APPLIED' in r['scenario_review_flags']; assert not r['scenario_validated']; assert 'CORPORATE_FUNDAMENTALS_INCOMPLETE' in r['scenario_review_blockers']
...
def test_rds_verified_two_share_ads_and_extreme_consensus_cap():
 ...
 assert r['economic_identity_status']=='VERIFIED_NORMALIZED'; assert r['identity_adr_ratio_applied']==2; assert r['scenario_validated']; assert r['bull_target_price']<=95.32*1.60; assert 'CONSENSUS_HIGH_WINSORIZED_FOR_PLAUSIBILITY' in r['scenario_review_flags']; assert 'SCENARIO_FUNDAMENTALS_NORMALIZED_TO_TRADED_SECURITY' in r['scenario_review_flags']
