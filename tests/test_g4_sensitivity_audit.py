...
def test_robust_hold_when_even_loosest_grid_has_no_pass():
    frame=pd.DataFrame([
        ...
        {'cedear_ticker':'RISKY','g4_status':'G4_FAIL_DOWNSIDE','bear_return_net':-.30,'expected_return_net':.20,'uncertainty_penalty':0.,'correlation_penalty':0.,'concentration_penalty':0.,'minimum_margin_over_hurdle':.02,'required_margin_over_cash':.04,'valuation_engine_type':'EQUITY'},
...
def test_frontier_is_detected_without_mutating_production_policy():
    ...
    assert ((grid['bear_limit']==-.20)&(grid['cash_hurdle']==.05)&(grid['uncertainty_scale']==0)&(~grid['dynamic_equity_margin'])&(grid['pass_count']==1)).any()
