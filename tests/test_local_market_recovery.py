...
def test_missing_local_quote_gets_analytical_reference_but_not_fake_price():
    frame = pd.DataFrame([_base_row(analytical_local_ref_ars=None, implied_ccl=None, market_ccl_crosscheck_status="MARKET_CCL_CROSSCHECK_UNAVAILABLE", ccl_status="CCL_BLOCKED", valuation_g4_local_gate="BLOCKED", last_price_ars=None, bid_ars=None, ask_ars=None)])
...
def test_bad_observed_quote_uses_theoretical_reference_for_analytical_g4():
    frame = pd.DataFrame([_base_row(analytical_local_ref_ars=10.0, implied_ccl=1.0, market_ccl_crosscheck_status="MARKET_CCL_CROSSCHECK_BLOCKED", ccl_status="CCL_BLOCKED_MARKET_DEVIATION", valuation_g4_local_gate="BLOCKED")])
    out = recover_analytical_local_market(frame).iloc[0]
...
def test_canonical_ratio_can_be_reconciled_independently_to_market_ccl():
    frame = pd.DataFrame([_base_row(ratio_status="RATIO_UNVERIFIED_COMAFI", ratio_used=2.0, universe_ratio=2.0, last_close=50.0, analytical_local_ref_ars=40000.0, market_ccl_reference=1600.0, implied_ccl=1600.0, ccl_status="CCL_READY_UNVERIFIED", valuation_g4_local_gate="BLOCKED")])
    out = recover_analytical_local_market(frame).iloc[0]
...
def test_bad_canonical_ratio_is_not_recovered():
    frame = pd.DataFrame([_base_row(ratio_status="RATIO_UNVERIFIED_COMAFI", ratio_used=10.0, universe_ratio=10.0, last_close=100.0, analytical_local_ref_ars=1000.0, market_ccl_reference=1600.0, implied_ccl=100.0, market_ccl_crosscheck_status="MARKET_CCL_CROSSCHECK_BLOCKED", ccl_status="CCL_READY_UNVERIFIED", valuation_g4_local_gate="BLOCKED")])
    out = recover_analytical_local_market(frame).iloc[0]
