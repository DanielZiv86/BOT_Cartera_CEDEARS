...
def _geo_policy():
    return {"allowed_us_exchanges":["NYSE","NEW YORK","NASDAQ","NASDAQ GS","NASDAQ GM","NASDAQ CM","NYSE ARCA","CBOE BZX"],"allowed_issuer_countries":{"US":"UNITED_STATES","ES":"EUROPE","GB":"EUROPE"},"us_listed_fund_instrument_types":["ETF TRADICIONAL","EXPOSICION ESPECIAL"],"mandate_exceptions":["IWDA"],"reason_codes":{"eligible":"ISSUER_US_OR_EUROPE_AND_US_TRADED","us_listed_fund":"US_LISTED_FUND_VEHICLE","country_unknown":"ISSUER_COUNTRY_UNVERIFIED","country_outside_mandate":"ISSUER_OUTSIDE_US_EUROPE","exchange_outside_mandate":"UNDERLYING_NOT_US_TRADED","mandate_exception":"EXPLICIT_USER_MANDATE_EXCEPTION"}}
...
def test_geography_accepts_us_and_european_issuers_trading_in_us():
    frame=pd.DataFrame([{"cedear_ticker":"JPM","issuer_country":"US","underlying_market":"New York","instrument_type":"Acción","eligible_for_research":True},{"cedear_ticker":"SAN","issuer_country":"ES","underlying_market":"New York","instrument_type":"ADR","eligible_for_research":True},{"cedear_ticker":"AAPL","issuer_country":"ESTADOS UNIDOS","underlying_market":"NASDAQ GS","instrument_type":"Acción","eligible_for_research":True}]); out=apply_geography_eligibility(frame,_geo_policy()); assert out["geography_eligible"].tolist()==[True,True,True]

def test_geography_rejects_brazil_even_when_security_trades_in_us():
    frame=pd.DataFrame([{"cedear_ticker":"PBR","issuer_country":"BR","underlying_market":"New York","instrument_type":"ADR","eligible_for_research":True}]); out=apply_geography_eligibility(frame,_geo_policy()); row=out.iloc[0]; assert not bool(row["geography_eligible"]); assert row["geography_eligibility_reason"]=="ISSUER_OUTSIDE_US_EUROPE"; assert not bool(row["eligible_for_research"])

def test_geography_fails_closed_on_unknown_country_for_corporate_security():
    frame=pd.DataFrame([{"cedear_ticker":"XYZ","issuer_country":"","underlying_market":"NYSE","instrument_type":"Acción","eligible_for_research":True}]); out=apply_geography_eligibility(frame,_geo_policy()); assert not bool(out.iloc[0]["eligible_for_research"]); assert out.iloc[0]["geography_eligibility_reason"]=="ISSUER_COUNTRY_UNVERIFIED"

def test_geography_accepts_us_listed_fund_without_corporate_country():
    frame=pd.DataFrame([{"cedear_ticker":"SPY","issuer_country":"","underlying_market":"NYSE ARCA","instrument_type":"ETF tradicional","eligible_for_research":True},{"cedear_ticker":"QQQ","issuer_country":"","underlying_market":"NASDAQ GM","instrument_type":"ETF tradicional","eligible_for_research":True}]); out=apply_geography_eligibility(frame,_geo_policy()); assert out["geography_eligible"].tolist()==[True,True]; assert set(out["geography_eligibility_reason"])=={"US_LISTED_FUND_VEHICLE"}

def test_geography_preserves_iwda_explicit_exception():
    frame=pd.DataFrame([{"cedear_ticker":"IWDA","issuer_country":"","underlying_market":"EUROCLEAR","instrument_type":"ETF tradicional","eligible_for_research":True,"mandate_exception":True}]); out=apply_geography_eligibility(frame,_geo_policy()); assert bool(out.iloc[0]["eligible_for_research"]); assert out.iloc[0]["geography_eligibility_reason"]=="EXPLICIT_USER_MANDATE_EXCEPTION"
