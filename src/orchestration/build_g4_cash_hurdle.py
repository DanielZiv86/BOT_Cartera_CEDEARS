...
def _govern(m):
    x=dict(m); blocked=int(x.get('blocked_count',0) or 0); passes=int(x.get('pass_count',0) or 0); clean=int(x.get('clean_g4_pass_count',passes) or 0); review=int(x.get('extreme_target_review_required_count',0) or 0); total=int(x.get('ticker_count',0) or 0)
    if total==0 or blocked>0: x.update(deployment_decision='RESEARCH_BLOCKED',ranking_status='PARTIAL_NOT_ACTIONABLE')
...
def _dynamic_equity_margin(result,reviewed,raw_policy):
    ...
    meta=reviewed[[c for c in ['cedear_ticker','valuation_engine_type','scenario_validated'] if c in reviewed.columns]].drop_duplicates('cedear_ticker')
    out=result.merge(meta,on='cedear_ticker',how='left')
    req=[]
    for _,r in out.iterrows():
        ...
        req.append(margin)
    out['required_margin_over_cash']=req
    mask=(out['g4_status']=='G4_PASS')&(out['net_benefit_vs_cash']<out['required_margin_over_cash'])
    out.loc[mask,'g4_status']='G4_FAIL_RETURN'; out.loc[mask,'g4_reason']='INSUFFICIENT_DYNAMIC_EQUITY_MARGIN_VS_CASH'
    return out
...
def main():
    ...
    p.add_argument('--scenario-policy',default='config/scenario_review_policy.yml');p.add_argument('--brokerage-rate',type=float,default=.006);a=p.parse_args()
    ...
    invalid=~reviewed.get('scenario_validated',pd.Series(False,index=reviewed.index)).fillna(False)
    reviewed.loc[invalid,'valuation_status']='BLOCKED_BY_DATA'
    result,metrics=calculate_g4_cash_hurdle(local_market=local,valuation_inputs=reviewed,portfolio_fit=fit,positions=positions,policy=policy,brokerage_rate=a.brokerage_rate)
    result=_dynamic_equity_margin(result,reviewed,raw)
    # Recompute G4 counts after dynamic margin before governance/review.
    ...
    reviewed_json.to_json(out/'scenario_review.json',orient='records',indent=2,force_ascii=False)
    json_df=result.copy()
    ...
    pq.to_parquet(out/'g4_cash_hurdle.parquet',index=False)
    ranked=result[result['g4_status']!='BLOCKED_BY_DATA'].sort_values('net_benefit_vs_cash',ascending=False);ranked.to_json(out/'g4_ranked_evaluated.json',orient='records',indent=2,force_ascii=False)
    ...
    manifest={'layer':'Deep Scenario Review + G4 Cash Hurdle','created_at':datetime.now(timezone.utc).isoformat(),'policy_file':a.policy,'scenario_policy_file':a.scenario_policy,'policy':raw,'brokerage_rate_per_side':a.brokerage_rate,**metrics,'methodology_version':'G4-2.0'};(out/'g4_manifest.json').write_text(json.dumps(manifest,indent=2,ensure_ascii=False),encoding='utf-8');print(json.dumps(manifest,indent=2));return 0
