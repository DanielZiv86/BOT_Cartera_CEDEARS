...
def main():
    ...
    # V2.1 geography is sourced from Banco Comafi's official CEDEAR catalogue
    # ("País" / "País de Origen"), not from an external market-data provider.
    official_eligible["issuer_country"]=official_eligible.get("country_of_origin",pd.Series("",index=official_eligible.index)).fillna("").astype(str)
    official_eligible["issuer_country_source"]="COMAFI_COUNTRY_OF_ORIGIN"
    official_eligible=apply_geography_eligibility(official_eligible,load_geography_policy(args.geography_policy))
    geography_excluded=official_eligible[~official_eligible["geography_eligible"]].copy(); eligible=official_eligible[official_eligible["eligible_for_research"]].copy()
    ...
    geography_accounting_ok=len(official_eligible)==len(eligible)+len(geography_excluded); gate_ok=symbol_map_ok and blocking_unresolved.empty and not missing_mandate_exceptions and geography_accounting_ok and len(eligible)>0

    out=Path(args.output_dir); out.mkdir(parents=True,exist_ok=True)
    eligible.to_parquet(out/"cedear_universe_master.parquet",index=False); eligible.to_json(out/"cedear_universe_master.json",orient="records",indent=2,force_ascii=False); geography_excluded.to_json(out/"cedear_geography_exclusions.json",orient="records",indent=2,force_ascii=False)
    canonical[canonical["user_mandate_excluded"]].to_json(out/"cedear_mandate_exclusions.json",orient="records",indent=2,force_ascii=False); canonical[canonical["universe_override_inclusion"]==True].to_json(out/"cedear_universe_inclusions.json",orient="records",indent=2,force_ascii=False); unresolved.to_json(out/"cedear_official_reconciliation_unresolved.json",orient="records",indent=2,force_ascii=False); blocking_unresolved.to_json(out/"cedear_official_reconciliation_blocking.json",orient="records",indent=2,force_ascii=False)
