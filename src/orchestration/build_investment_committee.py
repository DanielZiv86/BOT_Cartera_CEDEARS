...
def main()->None:
    ...
    elif int(gm.get('blocked_count',0))>0:decision='HOLD_CASH_NO_ACTION';rationale='SCENARIO_DATA_GAPS_FAIL_CLOSED'
    ...
    now=datetime.now(timezone.utc).isoformat();committee={'layer':'INVESTMENT_COMMITTEE','methodology_version':'COMMITTEE-1.1','created_at':now,'decision':decision,'rationale':rationale,'risk_veto':rr.get('risk_veto'),'g4_cash_optimality_status':gm.get('cash_optimality_status'),'g4_pass_count':gm.get('pass_count'),'g4_fail_count':gm.get('fail_count'),'g4_blocked_count':gm.get('blocked_count'),'new_trades':[],'fabricated_trade_count':0,'decision_is_valid_no_action':decision=='HOLD_CASH_NO_ACTION'};shadow={'created_at':now,'e2e_run_id':gl['e2e_run_id'],'committee_run_id':a.committee_run_id,'decision':decision,'orders':[],'order_count':0,'status':'NO_ACTION_PERSISTED' if not committee['new_trades'] else 'PENDING_EXECUTION_GATE'}
