...
class NonEquityTrackerConnector:
    def __init__(self, tracker_policy: dict[str, Any], timeout: int = 30) -> None:
        ...
        self.session.headers.update({"User-Agent": "CEDEAR-NonEquity-Tracker/1.0"})
    ...
    def _parse_ishares(self, ticker: str, cfg: dict[str, Any], html: str) -> TrackerSnapshot:
        # iShares pages expose labels like "NAV as of Aug 31, 2026" followed by the value.
        nav = None
        nav_date = None
        market = None
        market_date = None
        ...
        m = re.search(r"NAV\s+as\s+of\s+([A-Za-z]{3}\s+\d{1,2},?\s+\d{4}).{0,400}?\$\$?\s*([0-9][0-9,]*(?:\.[0-9]+)?)", html, re.I | re.S)
        if m:
            nav_date = self._normalize_date(m.group(1))
            nav = self._money(m.group(2))

        m = re.search(r"Closing\s+Price.{0,300}?\$\s*([0-9][0-9,]*(?:\.[0-9]+)?).{0,250}?as\s+of\s+([A-Za-z]{3}\s+\d{1,2},?\s+\d{4})", html, re.I | re.S)
        if m:
            market = self._money(m.group(1))
            market_date = self._normalize_date(m.group(2))

        m = re.search(r"Premium/Discount.{0,250}?(-?[0-9]+(?:\.[0-9]+)?)", html, re.I | re.S)
        if m:
            try:
    ...
    def _parse_ssga(self, ticker: str, cfg: dict[str, Any], html: str) -> TrackerSnapshot:
        ...
        # Prefer the Fund Net Asset Value section because the page may repeat NAV in several places.
