...
def build_equity_scenario(symbol: str, current_price: float | None, connector: FinnhubConnector, policy: dict[str, Any], as_of: date | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if ticker in (policy.get("index_etf_models", {}) or {}):
            return build_index_etf_scenario(ticker, current_price, policy)
        code = str(exc) if isinstance(exc, FinnhubAccessDenied) else "FINNHUB_REQUEST_FAILED"
        return {"underlying_ticker": ticker, "valuation_method": "FINNHUB_ANALYST_CONSENSUS_SCREENING_V2", "valuation_status": "BLOCKED_BY_DATA", "valuation_confidence": None, "blockers": [code], "source_ref": "Finnhub /stock/price-target", "retrieved_at": connector.retrieved_at()}
    ...
    common = {"underlying_ticker": ticker, "valuation_method": "FINNHUB_ANALYST_CONSENSUS_SCREENING_V2", "current_price": current_price, "analyst_count": analyst_count, "price_target_last_updated": last_updated, "price_target_age_days": pt_age, "recommendation_period": None, "recommendation_age_days": None, "latest_fundamental_period": None, "fundamentals_age_days": None, "enrichment_status": "DEFERRED_TO_DEEP_RESEARCH", "source_date": last_updated, "source_ref": "Finnhub /stock/price-target", "retrieved_at": connector.retrieved_at()}

    if blockers:
        if ticker in (policy.get("index_etf_models", {}) or {}):
            aggregate = build_index_etf_scenario(ticker, current_price, policy)
            if aggregate.get("valuation_status") == "VALUATION_READY":
                aggregate["analyst_consensus_blockers"] = blockers
                aggregate["enrichment_status"] = "INDEX_AGGREGATE_FUNDAMENTALS_FALLBACK"
                aggregate["retrieved_at"] = connector.retrieved_at()
                return aggregate
