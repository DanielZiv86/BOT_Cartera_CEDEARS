...
def build_equity_scenario(symbol: str, current_price: float | None, connector: FinnhubConnector, policy: dict[str, Any], as_of: date | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {'underlying_ticker':ticker,'valuation_method':'FINNHUB_ANALYST_CONSENSUS_SCREENING_V3','valuation_status':'BLOCKED_BY_DATA','valuation_confidence':None,'blockers':[code],'source_ref':'Finnhub /stock/price-target','retrieved_at':connector.retrieved_at()}
    ...
    common={'underlying_ticker':ticker,'valuation_method':'FINNHUB_ANALYST_CONSENSUS_SCREENING_V3','current_price':current_price,'analyst_count':analyst_count,'consensus_target_high':high,'consensus_target_mean':mean,'consensus_target_median':median,'consensus_target_low':low,'price_target_last_updated':last_updated,'price_target_age_days':pt_age,'fundamental_eps_normalized':eps,'fundamental_pe_normalized':pe,'fundamental_eps_growth_3y':eps_growth,'fundamental_debt_to_equity':debt_equity,'fundamental_net_margin':net_margin,'fundamental_current_ratio':current_ratio,'fundamental_source_error':fundamental_error,'fundamental_source_ref':'Finnhub /stock/metric?metric=all','enrichment_status':'FUNDAMENTALS_ATTACHED_FOR_DEEP_SCENARIO_REVIEW','source_date':last_updated,'source_ref':'Finnhub /stock/price-target + /stock/metric','retrieved_at':connector.retrieved_at()}
