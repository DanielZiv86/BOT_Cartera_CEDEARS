...
def _ensure_identity_schema(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    defaults={"program_name":"","cedear_byma_symbol":"","underlying_symbol":"","caja_code":"","isin_cedear":"","country_of_origin":"","official_issuer":"","official_source_url":"","official_source_kind":""}
    ...
    out["country_of_origin"]=out["country_of_origin"].map(_norm)
    return out
...
def _parse_identity_tables(url: str, issuer: str, kind: str, min_rows: int=5) -> pd.DataFrame:
    ...
    for table in tables:
        ...
        candidates.append(out[["program_name","cedear_byma_symbol","underlying_symbol","caja_code","isin_cedear","country_of_origin","official_issuer","official_source_url","official_source_kind"]])
...
def _coalesce_official_identity_rows(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for _,group in frame[frame["_key"].ne("SYM:")].groupby("_key",sort=False):
        rec={}
        for col in ["program_name","cedear_byma_symbol","underlying_symbol","caja_code","isin_cedear","country_of_origin"]:
...
def reconcile_official_identity(source: pd.DataFrame,audit: OfficialUniverseAudit) -> pd.DataFrame:
    ...
    for symbol,group in official[official["cedear_byma_symbol"].ne("")].groupby("cedear_byma_symbol"):
        values=[_norm(v) for v in group["country_of_origin"] if _norm(v)]
        if values: country_by_symbol[_norm_symbol(symbol)]=values[0]
    out["country_of_origin"]=out["cedear_byma_symbol"].map(lambda s:country_by_symbol.get(_norm_symbol(s),""))
    out["issuer_country_source"]="COMAFI_COUNTRY_OF_ORIGIN"
