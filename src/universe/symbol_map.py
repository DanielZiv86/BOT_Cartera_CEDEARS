...
def load_alias_config(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict): raise ValueError("Alias config must be a mapping")
    return payload


def normalize_provider_symbol(underlying_ticker: str, provider: str, aliases: dict[str, Any], underlying_market: str | None = None) -> str:
    ticker = underlying_ticker.strip(); provider_aliases = aliases.get("providers", {}).get(provider, {})
    if ticker in provider_aliases: return str(provider_aliases[ticker])
    ...
    if underlying_market and underlying_market in market_rules:
        ...
        if suffix and not ticker.endswith(suffix): return f"{ticker}{suffix}"
    return ticker
...
def build_symbol_map(eligible_universe: pd.DataFrame, aliases: dict[str, Any], providers: tuple[str, ...] = ("yahoo", "stooq", "iol", "data912")) -> pd.DataFrame:
    ...
    for row in eligible_universe.to_dict(orient="records"):
        ...
        for provider in providers:
            ...
            # providers receive the underlying symbol. This prevents BBVA/BBV-type bugs.
            if provider in {"iol","data912"}: base[f"{provider}_symbol"] = local
            else: base[f"{provider}_symbol"] = normalize_provider_symbol(str(row["underlying_ticker"]),provider,aliases,row.get("underlying_market"))
        records.append(base)
