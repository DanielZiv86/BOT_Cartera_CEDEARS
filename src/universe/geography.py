...
def apply_geography_eligibility(frame: pd.DataFrame, policy: dict[str, Any]) -> pd.DataFrame:
    ...
    Issuer origin is expected from Banco Comafi's official ``País`` / ``País de Origen``
    field. Unknown or non-US/non-European origins fail closed. IWDA remains the sole
    explicit mandate exception.
    """
    out=frame.copy(); countries={_norm(k):str(v) for k,v in (policy.get("allowed_issuer_countries") or {}).items()}; allowed_exchanges={_norm(x) for x in policy.get("allowed_us_exchanges",[])}; exceptions={_norm(x) for x in policy.get("mandate_exceptions",[])}; reasons=policy.get("reason_codes",{}) or {}; allowed_regions={"UNITED_STATES","EUROPE"}
    geo_ok=[]; geo_reason=[]; regions=[]; normalized_countries=[]
    for _,row in out.iterrows():
        ticker=_norm(row.get("cedear_ticker") or row.get("legacy_cedear_ticker")); raw_country=row.get("issuer_country"); country=_country_code(raw_country); exchange=_norm(row.get("underlying_market")); normalized_countries.append(country)
    ...
    out["issuer_country_normalized"]=normalized_countries; out["issuer_region"]=regions; out["geography_eligible"]=geo_ok; out["geography_eligibility_reason"]=geo_reason; out["eligible_for_research"]=out["eligible_for_research"].eq(True) & out["geography_eligible"].eq(True)
    return out
