...
def _build_exposure_table(
    ...
    category_col: str,
    cash_weight: float,
    risky_weight_input: float,
) -> pd.DataFrame:
    if mapping is None or not {"cedear_ticker", category_col}.issubset(mapping.columns):
        return pd.DataFrame()
    exposure = pos.merge(mapping[["cedear_ticker", category_col]], on="cedear_ticker", how="left")
    exposure[category_col] = exposure[category_col].fillna("UNKNOWN")
    exposure = exposure.groupby(category_col, as_index=False)["weight"].sum()
    exposure["risky_sleeve_weight"] = exposure["weight"] / risky_weight_input if risky_weight_input > 0 else 0.0
    if cash_weight > 0:
        exposure = pd.concat([
            exposure,
            pd.DataFrame([{category_col: "Cash", "weight": float(cash_weight), "risky_sleeve_weight": 0.0}]),
        ], ignore_index=True)
    return exposure.sort_values("weight", ascending=False).reset_index(drop=True)
...
def calculate_portfolio_risk(
    ...
    if pos.empty:
        return ({"portfolio_risk_status": "BLOCKED_BY_PORTFOLIO_DATA", "reason": "NO_POSITIVE_WEIGHTS"}, pd.DataFrame(), pd.DataFrame(), pd.DataFrame())
    ...
    if len(tickers) != len(pos):
        ...
        return ({"portfolio_risk_status": "BLOCKED_BY_PORTFOLIO_DATA", "reason": "PORTFOLIO_TICKERS_MISSING_FROM_RETURN_MATRIX", "missing_tickers": missing_tickers}, pd.DataFrame(), pd.DataFrame(), pd.DataFrame())
    ...
    if covariance_matrix.isna().any().any():
        return ({"portfolio_risk_status": "BLOCKED_BY_RISK_DATA", "reason": "INCOMPLETE_COVARIANCE_MATRIX"}, pd.DataFrame(), pd.DataFrame(), pd.DataFrame())
    ...
    risk_contribution_pct = component_variance / portfolio_var if portfolio_var > 0 else np.zeros_like(component_variance)
    ...
    sector_exposure = _build_exposure_table(pos, sector_map, "sector", cash_weight, risky_weight_input)
    factor_exposure = _build_exposure_table(pos, factor_map, "factor", cash_weight, risky_weight_input)

    noncash_sector = sector_exposure[sector_exposure["sector"] != "Cash"] if not sector_exposure.empty else pd.DataFrame()
    noncash_factor = factor_exposure[factor_exposure["factor"] != "Cash"] if not factor_exposure.empty else pd.DataFrame()
    ...
    summary = {
        ...
        "largest_non_cash_sector_nav_weight": float(noncash_sector["weight"].max()) if not noncash_sector.empty else None,
        "largest_non_cash_sector_risky_sleeve_weight": float(noncash_sector["risky_sleeve_weight"].max()) if not noncash_sector.empty else None,
        "largest_non_cash_factor_nav_weight": float(noncash_factor["weight"].max()) if not noncash_factor.empty else None,
        "largest_non_cash_factor_risky_sleeve_weight": float(noncash_factor["risky_sleeve_weight"].max()) if not noncash_factor.empty else None,
        "classification_concentration_scope": "TOP_LEVEL_INSTRUMENT_BUCKET_NOT_ETF_LOOKTHROUGH",
    }
