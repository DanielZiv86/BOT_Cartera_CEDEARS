...
- **Validate any economic/policy change against real market data before opening a PR**, via the real production GitHub Actions workflow chain (`weekly_screening.yml` -> `valuation_scenarios.yml` with `dispatch_g4=true` -> G4 -> Risk -> Committee), dispatched on the feature branch — see the "pinned lineage" note in `PROJECT_STATE.md` for why this works pre-merge without needing `main`. (The shadow-comparison chain this line used to point to, `value_research_shadow.yml`, was retired 2026-09-14 once production itself became value-first — its whole purpose was moot.) Quote the before/after numbers in the PR body.
